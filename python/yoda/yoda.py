...
def tobytes( contents ):
  if is_py3:
    if isinstance( contents, unicode ):
      return unicode.encode( contents )
  return contents


def tounicode( s, coding='utf-8' ):
...
def cx2str( cx_string ):
  char_p = L.clang_getCString( cx_string )
...
class CodeCompletionResults( CIndexWrap ):
  ...
  def binrange( self, query ):
    ...
    if query:
      ...
      if query[-1] == 'z': # TODO: Fix this ugly solution.
        nextq = query[:-1] + unichr( 0x7f )
      else:
        nextq = query[:-1] + unichr( ord( query[-1] ) + 1 )
  ...
  def binsearch( self, query, lo=0, hi=None ):
    ...
    p = self.bisect_left( query, lo, hi )
    while p != hi and _ClangCmpKey( self[ p ].spelling ) == cmp_query:
      if self[ p ].spelling == query: # exactly match the query
        return p
      p += 1
    else:
      return -1
...
def _ClangCmpKey( s ):
  ...
  # order. Non alphabetical characters are preordered from alphatbets.
  import string
  ...
  if is_py3:
    ...
    #                           non_letters_repl + string.ascii_uppercase )
    # for unicode
    trans = dict( (ord(s.decode('utf-8')), r.decode('utf-8'))
                  for s, r in zip( non_letters      + string.ascii_letters,
                                   non_letters_repl + string.ascii_uppercase ) )
  def actual( x ):
    if not is_py3 and isinstance( x, str ):
      x = x.decode( 'utf-8' )
